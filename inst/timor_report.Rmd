...
trips_per_week <- timor.report::trips_per_week
...
```{r TripWeek, echo=FALSE, message=FALSE, warning=FALSE, fig.height=4, fig.width=6.5, fig.cap='(ref:TripWeek)'}
...
trips_per_week %>%
  ...
  geom_line(aes(color = area), size = 0.15) +
...
```{r MonthlyCatch, echo=FALSE, message=FALSE, warning=FALSE, fig.height=7, fig.width=6.5, fig.cap='(ref:MonthlyCatch)'}
...
mean_cpue <-
  ...
  dplyr::group_by(area, month_period) %>%
  dplyr::summarise(
    mean_cpue = mean(catch_effort, na.rm = TRUE)
  )
...
trip_effort <-
  ...
  dplyr::group_by(area, month_period) %>%
  dplyr::summarise(
    median_effort = median(effort, na.rm = TRUE)
  )


total_catch_month <-
  dplyr::left_join(mean_cpue, trip_effort, by = c("area", "month_period")) %>%
  dplyr::left_join(trips_per_week, by = c("area", "month_period")) %>%
  dplyr::rename(trips_month = sum) %>%
  dplyr::select(area:trips_month) %>%
  dplyr::mutate(tot_catch = (mean_cpue * median_effort * trips_month) / 1000)

p1 <-
  total_catch_month %>%
  ggplot(aes(month_period, tot_catch)) +
  theme_minimal(13) +
  annotate("rect",
    xmin = as.Date("2020-03-28"), xmax = as.Date("2021-12-31"),
    ymin = -Inf, ymax = Inf, fill = "grey", alpha = .5
  ...
  geom_point(aes(color = area), size = 0.25) +
  geom_line(aes(color = area), size = 0.15) +
  stat_smooth(aes(fill = area, color = area), method = "loess", alpha = 0.25, size = 0.75) +
  scale_fill_manual(values = area_colors) +
  scale_color_manual(values = area_colors) +
  scale_y_continuous(
    expand = c(0, 0),

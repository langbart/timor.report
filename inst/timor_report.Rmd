...
The trip effort (fisher-hours) of the three regions appears to be quite similar (fig. \@ref(fig:TripEffort)a). The trip effort mostly ranges between two to ten fisher-hours, with some peaks reaching up to 28 fisher-hours. There seems to be no change in trip effort over time for Atauro island and the north coast, while the south coast shows a slight increase (fig. \@ref(fig:TripEffort)b).
...
The small pelagics and the cephalopods caught on Atauro island appear to be marginally bigger than on the north and south coast, whereas the large demersal caught on the south coast seem slightly bigger than on Atauro island and the north coast (fig. \@ref(fig:LengthArea)).
...
A pattern of seasonality is observed for the total number of trips taken per week as well as for the estimated total monthly landings in all three regions, with the strongest trend seen in the south coast (fig. \@ref(fig:WeekTrips); fig. \@ref(fig:WeekCatch)). The total number of trips per week decreased around April/May and increased again around September/October.
...
plot_dat <- 
  ...
  dplyr::summarise(mean = mean(weight, na.rm =TRUE),
                   ...
                   CI_lower=(mean + qt((1-0.95)/2, df=n-1)*sem),
                   CI_upper=(mean - qt((1-0.95)/2, df=n-1)*sem),
                   CI_upper = dplyr::case_when(catch_purpose == "Both" & .data$CI_upper > 10500 ~ 10500, 
                                               catch_purpose == "Own consumption" & .data$CI_upper > 750 ~ 750,
                                               catch_purpose == "Sold on market" & .data$CI_upper > 6000 ~ 6000,
                                               TRUE ~ .data$CI_upper),
...
report_data %>%
  ...
  dplyr::summarise(mean = mean(price_weight, na.rm =TRUE),
                   ...
                   CI_lower=(mean + qt((1-0.95)/2, df=n-1)*sem),
                   CI_upper=(mean - qt((1-0.95)/2, df=n-1)*sem)) %>%
  dplyr::mutate(CI_lower = dplyr::case_when(.data$CI_lower < 0 ~ 0, TRUE ~ .data$CI_lower),
                CI_upper = dplyr::case_when(gear_type == "spear gun" & .data$CI_upper > 12 ~ 12,
                                            TRUE ~ .data$CI_upper)) %>% 
  ggplot(aes(month_period, mean))+
...
report_data %>%
  ...
  dplyr::summarise(mean = mean(price_weight, na.rm =TRUE),
                   ...
                   CI_upper=(mean - qt((1-0.95)/2, df=n-1)*sem)) %>% 
  dplyr::mutate(CI_lower = dplyr::case_when(.data$CI_lower < 0 ~ 0, TRUE ~ .data$CI_lower),
                CI_upper = dplyr::case_when(fish_group == "Large demersals" &.data$CI_upper > 40 ~ 40,
                                            fish_group == "Shark and rays" &.data$CI_upper > 40 ~ 40,
                                            fish_group == "Small demersals" &.data$CI_upper > 40 ~ 40,
                                            fish_group == "Small pelagics" &.data$CI_upper > 12 ~ 12,
                                            fish_group == "Cephalopods" &.data$CI_upper > 50 ~ 50,
                                            TRUE ~ .data$CI_upper)) %>% 
  ggplot(aes(month_period, mean))+
  theme_minimal()+
